...
router.post("/", async (req, res) => {
  ...
  const newDrama = { title: drama.title, year: drama.year || "", genre: drama.genre || "", rating: drama.rating || 0, poster: drama.poster || "", notes: drama.notes || "", addedAt: new Date().toISOString() };
...
router.delete("/:memberName/drama/:idx", async (req, res) => {
  ...
  const dramas = (row.dramas as any[]).filter((_, i) => i !== idx);
  const [updated] = await db.update(drakorTable).set({ dramas, updatedAt: new Date() }).where(eq(drakorTable.memberName, memberName)).returning();
  return res.json(updated);
});
